  ...
  roles = flatten([
    for database in var.mongoDB.databases : [
      for role in database.roles : merge(role, {
        ...
        databaseId = "${azurerm_cosmosdb_account.main["mongo"].id}/sqlDatabases/${database.name}"
       }) if role.enable
    ] if database.enable
  ])
  users = flatten([
...
resource azurerm_cosmosdb_mongo_database mongo_db {
  ...
  resource_group_name = azurerm_cosmosdb_account.main["mongo"].resource_group_name
  account_name        = azurerm_cosmosdb_account.main["mongo"].name
...
resource azurerm_private_endpoint mongo_db {
  count               = var.mongoDB.enable ? 1 : 0
  name                = azurerm_cosmosdb_account.main["mongo"].name
  resource_group_name = azurerm_cosmosdb_account.main["mongo"].resource_group_name
  location            = azurerm_cosmosdb_account.main["mongo"].location
  subnet_id           = data.azurerm_subnet.data.id
  private_service_connection {
    name                           = azurerm_cosmosdb_account.main["mongo"].name
    private_connection_resource_id = azurerm_cosmosdb_account.main["mongo"].id
...
resource azurerm_mongo_cluster main {
  ...
  compute_tier           = var.mongoDBvCore.cluster.type
  version                = var.mongoDBvCore.cluster.version
  data_api_mode_enabled  = var.mongoDBvCore.cluster.dataApi.enable
  shard_count            = var.mongoDBvCore.node.count
  storage_type           = var.mongoDBvCore.node.storage.type
  storage_size_in_gb     = var.mongoDBvCore.node.storage.sizeGB
  high_availability_mode = var.mongoDBvCore.highAvailability.enable ? "ZoneRedundantPreferred" : "Disabled"
  authentication_methods = var.mongoDBvCore.cluster.authentication.methods
  administrator_username = var.mongoDBvCore.cluster.authentication.adminLogin.userName != "" ? var.mongoDBvCore.cluster.authentication.adminLogin.userName : data.azurerm_key_vault_secret.admin_username.value
  administrator_password = var.mongoDBvCore.cluster.authentication.adminLogin.userPassword != "" ? var.mongoDBvCore.cluster.authentication.adminLogin.userPassword : data.azurerm_key_vault_secret.admin_password.value
}
...
resource azurerm_private_endpoint mongo_cluster {
  count               = var.mongoDBvCore.enable ? 1 : 0
  name                = "${azurerm_mongo_cluster.main[0].name}-${azurerm_private_dns_zone_virtual_network_link.mongo_cluster[0].name}"
  resource_group_name = azurerm_resource_group.data_cosmos.name
  location            = azurerm_resource_group.data_cosmos.location
  subnet_id           = data.azurerm_subnet.data.id
  private_service_connection {
    name                           = azurerm_mongo_cluster.main[0].name
    private_connection_resource_id = azurerm_mongo_cluster.main[0].id
    is_manual_connection           = false
    subresource_names = [
      "MongoCluster"
    ]
