...
resource azurerm_network_interface job_manager {
  ...
  resource_group_name = azurerm_resource_group.job_manager[0].name
  location            = each.value.location
  edge_zone           = each.value.edgeZone
  ip_configuration {
    name                          = "ipConfig"
    private_ip_address_allocation = "Dynamic"
    subnet_id                     = data.azurerm_subnet.job_manager.id
  }
...
resource azurerm_linux_virtual_machine job_manager {
  ...
  network_interface_ids = [
    "${azurerm_resource_group.job_manager[0].id}/providers/Microsoft.Network/networkInterfaces/${each.value.name}"
  ]
  os_disk {
...
resource azurerm_virtual_machine_extension job_manager_initialize_linux {
  ...
  virtual_machine_id         = "${azurerm_resource_group.job_manager[0].id}/providers/Microsoft.Compute/virtualMachines/${each.value.name}"
...
resource azurerm_monitor_diagnostic_setting job_manager_monitor_linux {
  ...
  name                       = each.value.name
  target_resource_id         = "${azurerm_resource_group.job_manager[0].id}/providers/Microsoft.Compute/virtualMachines/${each.value.name}"
