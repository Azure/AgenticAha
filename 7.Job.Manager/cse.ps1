...
$taskAction   = New-ScheduledTaskAction -Execute "pwsh" -Argument "-NoProfile -NonInteractive -ExecutionPolicy Bypass -File $scriptFile -resourceGroupName ${autoScale.resourceGroupName} -jobManagerName ${autoScale.jobManagerName} -jobClusterName ${autoScale.jobClusterName} -jobClusterNodeLimit ${autoScale.jobClusterNodeLimit} -jobWaitThresholdSeconds ${autoScale.jobWaitThresholdSeconds} -workerIdleDeleteSeconds ${autoScale.workerIdleDeleteSeconds}"
