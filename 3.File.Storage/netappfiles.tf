...
locals {
  anfRegions = var.netAppFiles.enable ? distinct(concat([azurerm_resource_group.netapp_files[0].location], flatten([
  ...
  anfAccounts = [
    for region in local.anfRegions : merge(var.netAppFiles, {
      key  = region != azurerm_resource_group.netapp_files[0].location ? "${var.netAppFiles.name}-${region}" : var.netAppFiles.name
      name = region != azurerm_resource_group.netapp_files[0].location ? "${var.netAppFiles.name}-${region}" : var.netAppFiles.name
      resourceGroup = {
        name     = azurerm_resource_group.netapp_files[0].name
        location = region
      }
    })
  ]
  anfPools = flatten([
    for account in local.anfAccounts : [
  ...
  anfVolumes = flatten([
    for capacityPool in local.anfPools : [
      ...
      }) if volume.enable && (capacityPool.resourceGroup.location == azurerm_resource_group.netapp_files[0].location || volume.replication.enable)
    ]
  ])
  anfVolumesReplication = [
    for volume in local.anfVolumes : volume if volume.replication.enable
  ]
}

resource azurerm_netapp_account main {
  for_each = {
    for account in local.anfAccounts : account.key => account
  }
  ...
  depends_on = [
    azurerm_resource_group.netapp_files
  ]
}

resource azurerm_netapp_pool main {
  for_each = {
    for capacityPool in local.anfPools : capacityPool.key => capacityPool
  }
  name                    = each.value.name
  resource_group_name     = each.value.resourceGroup.name
  location                = each.value.resourceGroup.location
...
resource azurerm_netapp_volume main {
  for_each = {
    for volume in local.anfVolumes : volume.key => volume
  }
  name                            = each.value.name
  resource_group_name             = each.value.resourceGroup.name
  location                        = each.value.resourceGroup.location
  ...
  subnet_id                       = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/HPC.Network.${each.value.resourceGroup.location}/providers/Microsoft.Network/virtualNetworks/${var.virtualNetwork.name}/subnets/${var.virtualNetwork.subnetNameANF}"
  account_name                    = azurerm_netapp_account.main[each.value.accountName].name
  service_level                   = "Flexible"
  dynamic cool_access {
  ...
  dynamic data_protection_replication {
    for_each = each.value.replication.enable && each.value.resourceGroup.location != azurerm_resource_group.netapp_files[0].location ? [1] : []
    content {
      remote_volume_resource_id = "${azurerm_resource_group.netapp_files[0].id}/providers/Microsoft.NetApp/netAppAccounts/${var.netAppFiles.name}/capacityPools/${each.value.poolName}/volumes/${each.value.name}"
      remote_volume_location    = azurerm_resource_group.netapp_files[0].location
