...
resource azurerm_storage_container lustre_file {
  count              = var.lustreFiles.enable && var.lustreFiles.blobStorage.enable ? 1 : 0
  name               = var.lustreFiles.blobStorage.containerName.file
  storage_account_id = data.azurerm_storage_account.lustre_files[0].id
}

resource azurerm_storage_container lustre_log {
  count              = var.lustreFiles.enable && var.lustreFiles.blobStorage.enable ? 1 : 0
  name               = var.lustreFiles.blobStorage.containerName.log
  storage_account_id = data.azurerm_storage_account.lustre_files[0].id
}
...
resource azurerm_managed_lustre_file_system main {
  ...
  zones = [
    var.lustreFiles.availabilityZone.id
  ]
  identity {
    type = "UserAssigned"
    identity_ids = [
  ...
  dynamic root_squash {
    for_each = var.lustreFiles.rootSquash.enable ? [1] : []
    content {
      mode           = var.lustreFiles.rootSquash.mode
      squash_uid     = var.lustreFiles.rootSquash.userId
      squash_gid     = var.lustreFiles.rootSquash.groupId
      no_squash_nids = join(";", var.lustreFiles.rootSquash.trustedIps)
    }
  }
  dynamic hsm_setting {
    for_each = var.lustreFiles.blobStorage.enable ? [1] : []
    content {
      container_id         = azurerm_storage_container.lustre_file[0].id
      logging_container_id = azurerm_storage_container.lustre_log[0].id
      import_prefix        = var.lustreFiles.blobStorage.fileImport.prefix
    }
  }
  depends_on = [
