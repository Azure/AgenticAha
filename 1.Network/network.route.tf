...
locals {
  ...
  userDefinedRouteTableSubnets = flatten([
    for routeTable in local.userDefinedRouteTables : [
      ...
      }) if subnet.name != "GatewaySubnet" && subnet.name != "AzureBastionSubnet" && subnet.name != "AzureFirewallSubnet" && subnet.name != "AzureFirewallManagementSubnet"
    ]
  ])
}
...
resource azurerm_route_table main {
  for_each = {
    for routeTable in local.userDefinedRouteTables : routeTable.key => routeTable
  }
  name                = each.value.key
  ...
  dynamic route {
    ...
    content {
      ...
      next_hop_in_ip_address = route.value.nextHopAddress != "" ? route.value.nextHopAddress : null
    }
  }
  depends_on = [
...
resource azurerm_subnet_route_table_association main {
  for_each = {
    for subnet in local.userDefinedRouteTableSubnets : subnet.key => subnet
  }
