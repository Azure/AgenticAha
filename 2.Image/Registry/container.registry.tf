...
resource azurerm_container_registry main {
  ...
  retention_policy_in_days      = var.containerRegistry.retentionPolicy.enable ? var.containerRegistry.retentionPolicy.days : null
  ...
  dynamic network_rule_set {
    for_each = var.containerRegistry.tier == "Premium" ? [1] : []
    content {
      default_action = "Deny"
      ip_rule {
        action   = "Allow"
...
resource azurerm_private_dns_zone container_registry {
  count               = var.containerRegistry.tier == "Premium" ? 1 : 0
...
resource azurerm_private_endpoint container_registry {
  count               = var.containerRegistry.tier == "Premium" ? 1 : 0
  name                = "${lower(azurerm_container_registry.main.name)}-${azurerm_private_dns_zone_virtual_network_link.container_registry[0].name}"
