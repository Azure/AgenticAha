...
resource azurerm_shared_image main {
  for_each = {
    for imageDefinition in var.computeGallery.imageDefinitions : imageDefinition.name => imageDefinition if (lower(imageDefinition.type) == "linux") || (var.image.windows.enable && lower(imageDefinition.type) == "windows") || (!var.image.windows.enable && lower(imageDefinition.name) == "winuser")
  }
