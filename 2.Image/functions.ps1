...
function JoinActiveDirectory {
  ...
  if ($PSCmdlet.ParameterSetName -eq "Retry") {
    if ($activeDirectory.enable) {
      Retry 3 10 {
        JoinActiveDirectory -serverName $activeDirectory.domain.serverName -domainName $activeDirectory.domain.name -userName $activeDirectory.machine.adminLogin.userName -userPassword $activeDirectory.machine.adminLogin.userPassword
      }
    }
  } else {
    if ($userName -notlike "*@*") {
